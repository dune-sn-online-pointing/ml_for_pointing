...
Residuals:
  ΔX: {pred[idx, 0] - true[idx, 0]:+.3f}
  ΔY: {pred[idx, 1] - true[idx, 1]:+.3f}
  ΔZ: {pred[idx, 2] - true[idx, 2]:+.3f}"""
...
def plot_angular_error_vs_true_angles(predictions, fig):
    ...
    ax1.set_xlabel('True θ (degrees)', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Angular Error (degrees)', fontsize=12, fontweight='bold')
    ax1.set_title('Angular Error vs True θ', fontsize=14, fontweight='bold')
    ...
    ax2.set_xlabel('True φ (degrees)', fontsize=12, fontweight='bold')
    ax2.set_ylabel('Angular Error (degrees)', fontsize=12, fontweight='bold')
    ax2.set_title('Angular Error vs True φ', fontsize=14, fontweight='bold')
    ...
    ax3.set_xlabel('True θ (degrees)', fontsize=12, fontweight='bold')
    ax3.set_ylabel('True φ (degrees)', fontsize=12, fontweight='bold')
    ax3.set_title('Mean Angular Error in (θ, φ) Space', fontsize=14, fontweight='bold')
...
def generate_cosine_energy_pdf(predictions, results_dir, energy_bins=None):
    ...
    # Fill PDF for each energy bin using raw histograms (NO SMOOTHING)
    for i, (e_min, e_max) in enumerate(energy_bins):
        ...
        if n_events > 0:
            # Use raw histogram (no KDE smoothing)
    ...
    # Prepare output data
    pdf_data = {
        'pdf_2d': pdf_2d,  # Shape: (n_energy_bins, n_cosine_bins) - raw histogram, NO smoothing
    ...
    print(f"\n✅ 2D PDF saved to: {output_path}")
    print(f"   Shape: {pdf_2d.shape} (energy_bins × cosine_bins)")
...
def generate_comprehensive_analysis(results_dir, output_pdf=None):
    ...
    print(f"✓ Predictions: {len(predictions['angular_errors']):,} samples")
    ...
    with PdfPages(output_pdf) as pdf:
        ...
        print("📈 Generating page 1/7: Angular Error Analysis...")
        ...
        print("📈 Generating page 2/7: Training History...")
        fig = plt.figure(figsize=(14, 10))
        ...
        print("📈 Generating page 3/7: Component Correlations...")
        ...
        print("📈 Generating page 4/7: Energy-Dependent Performance...")
        fig = plt.figure(figsize=(14, 10))
        ...
        print("📈 Generating page 5/7: Cosine Similarity Analysis...")
        fig = plt.figure(figsize=(16, 10))
        ...
        print("📈 Generating page 6/7: Best and Worst Predictions...")
        ...
        print("📈 Generating page 7/7: Error vs True Angles...")
        fig = plt.figure(figsize=(14, 10))
    ...
    print("\n📊 Generating 2D cosine-energy PDF...")
    if 'energies' in predictions and 'cosine_similarity' in predictions:
        ...
        print("⚠️  Energy or cosine data not available, skipping PDF generation")
