#!/usr/bin/env python3
"""
Three-Plane Attention CNN Hyperparameter Optimization with Hyperopt
Uses attention mechanism and diverse architectures.
"""
...
def train_and_evaluate(params, data, config):
    ...
    test_u, test_v, test_x, test_y = test_data
...
def main():
    # Load configuration
    if len(sys.argv) < 2:
        print("Usage: python train_three_plane_attention_hyperopt.py <config.json>")
        sys.exit(1)
    ...
    print("="*70)
    print("THREE-PLANE ATTENTION CNN HYPERPARAMETER OPTIMIZATION")
    print("="*70)
    ...
    test_data = (test_u, test_v, test_x, test_y)
    ...
    model_name = config.get('model_name', 'three_plane_attention')
