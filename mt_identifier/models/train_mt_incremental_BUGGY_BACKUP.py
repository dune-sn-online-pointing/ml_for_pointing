...
def main():
    ...
    output_version = config.get('output', {}).get('version', 'mt_incremental')
    output_folder = os.path.join(output_base, output_version)
    ...
    # Model parameters
    model_params = config.get('training', {})
    ...
    # Data directories
    data_dirs = config.get('dataset', {}).get('data_directories', [])
    ...
    for file_path in test_files:
        ...
        if is_es and es_test_count < test_batch_size:
            ...
            for i in range(len(img)):
                ...
                meta = {"run": int(data.get("run", -1)), "event": int(data.get("event", -1)), "true_energy_sum": float(data.get("true_energy_sum", -1))}
                test_metadata_list.append(meta)
    ...
    test_metadata = test_metadata_list
    ...
    # Save predictions and metadata for analysis
    ...
    print(f"Predictions and metadata saved to: {output_dir}")
    print(f"{'='*80}")
