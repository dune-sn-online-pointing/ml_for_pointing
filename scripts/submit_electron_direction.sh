#!/bin/bash
#
# Submit Electron Direction Regressor Training to GPU
...
echo "========================================"
echo "Electron Direction GPU Training Submission"
echo "========================================"
...
if [ "$USE_CONDOR" = true ]; then
    # HTCondor submission
    CONDOR_SUB="$PROJECT_DIR/scripts/submit_condor_electron_direction.sub"
    ...
    TEMP_SUB=$(mktemp)
    cat "$CONDOR_SUB" | sed "s/^plane.*=.*/plane = $PLANE/" | sed "s/^max_samples.*=.*/max_samples = $MAX_SAMPLES/" > "$TEMP_SUB"
    ...
    # SSH submission
    REMOTE_CMD="cd $PROJECT_DIR && ./scripts/train_electron_direction.sh -j $JSON_CONFIG --plane $PLANE"
