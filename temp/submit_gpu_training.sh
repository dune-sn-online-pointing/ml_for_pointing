#!/bin/bash
#
# Submit Main Track Identifier Training to GPU
...
JSON_CONFIG="json/mt_identifier/production_training_prod_main.json"
...
Options:
    ...
    --json, -j <file>       JSON configuration file (default: $JSON_CONFIG)
...
if [ "$USE_CONDOR" = true ]; then
    # HTCondor submission
    CONDOR_SUB="$PROJECT_DIR/scripts/submit_condor_gpu.sub"
    ...
    REMOTE_CMD="cd $PROJECT_DIR && ./scripts/train_mt_identifier.sh -j \"$JSON_CONFIG\" --plane $PLANE"
